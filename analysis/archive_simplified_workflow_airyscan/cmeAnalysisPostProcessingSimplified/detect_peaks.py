...
def find_single_peaks_using_existing_model(analysis_metadata,
                                           experiment_group):
...
#     analysis_metadata = np.load(path_outputs+'/dataframes/analysis_metadata.npy', allow_pickle=True)    
    ...
    df = pd.read_csv(path_outputs + '/dataframes/' + df_name + '.zip')
    
    track_name = analysis_metadata.item().get('experiment_groups')[experiment_group]['tracks']
    
    index_DNM2positive = analysis_metadata.item().get('index_DNM2positive')
    number_of_track_splits = analysis_metadata.item().get('experiment_groups')[experiment_group]['number_of_track_splits']
    ...
    df.to_csv(path_outputs+'/dataframes/'+df_name+'.zip', index=False,
...
def identify_single_peaked_dnm2_events(analysis_metadata,
                                       experiment_group,
...
#     distances_sweep = np.arange(1,20,1) # minimum peak to peak distances
#     heights_sweep = np.arange(50,300,25) # minimum peak heights
#     widths_sweep = np.arange(1,10,1) # minimum peak widths

    # create a 3-D mesh of all possible combinations of peak-finding parameters
    distances_mesh_3, heights_mesh_3, widths_mesh_3 = np.meshgrid(distances_sweep,
                                                                  heights_sweep, 
                                                                  widths_sweep,
                                                                  indexing='ij')
    ...
    distance_best_fit = distances_sweep[ind_best_fit[0]]
    height_best_fit = heights_sweep[ind_best_fit[1]]
    width_best_fit = widths_sweep[ind_best_fit[2]]
