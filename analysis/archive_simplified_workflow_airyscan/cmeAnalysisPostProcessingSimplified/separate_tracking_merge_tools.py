...
def build_trees_from_directory(analysis_metadata,
                               experiment_group,
                               identifier_strings,
                               track_categories,
                               channel_for_trees,
                               channel_number_folder_label,
                               length_of_movie,
                               name_structures):
    
    path_outputs = analysis_metadata.item().get('path_outputs')
    
    path_tracks = analysis_metadata.item().get('experiment_groups')[experiment_group]['tracks_secondary']
...
#     print(temp_paths)
    ...
    for exp_number, exp in enumerate(all_track_paths):
        ...
        current_tracks = load_tracks(path_tracks + '/' + exp + '/Ch'+str(channel_number_folder_label)+'/Tracking/ProcessedTracks.mat')
        ...
        kd_tree_experiment, vals_tree = build_kd_tree_channel(current_tracks,
                                                              channel_for_trees,
                                                              length_of_movie)
        with open(path_outputs+'/dataframes/'+name_structures+'_kdtree_'+str(exp_number), 'wb') as f:
            pickle.dump(kd_tree_experiment, f)                

        np.save(path_outputs+'/dataframes/'+name_structures+'_kdtree_index_matrix_'+str(exp_number), vals_tree)

    print('saving tracks...\n')
    # split tracks
    number_of_track_splits = len(tracks)
    
    analysis_metadata.item().get('experiment_groups')[experiment_group]['name_secondary_structures'] = name_structures
...
#     split_valid_tracks = np.array_split(np.array(list(merged_all_tracks)),number_of_track_splits)
    ...
    for i in range(len(tracks)):
        ...
        np.save(path_outputs+"/dataframes/"+name_structures+"_"+str(i), np.array(list(tracks[i])))
