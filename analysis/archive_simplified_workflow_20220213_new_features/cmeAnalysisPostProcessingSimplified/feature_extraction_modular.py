...
class TrackFeatures:
    ...
    def __init__(self, 
        ...
        self.feature_options = ['lifetime',
                                ...
                                'fraction_peak_chA',
                                'initial_intensity_chA',
                                'initial_displacement_chA']
    ...
    def initial_intensity_chA(self, channel):
        """Extract the first detected intensity."""
        for i,track in enumerate(self.tracks):

            chA_amplitude = return_track_attributes.return_track_amplitude_no_buffer_channel(self.tracks, i, channel)
            self.feature_matrix.append(chA_amplitude[0])
            
    def initial_displacement_chA(self, channel):
        """Extract the first detected intensity."""
        for i,track in enumerate(self.tracks):

            x_pos = return_track_attributes.return_puncta_x_position_no_buffer_one_channel(self.tracks, i, channel)
            y_pos = return_track_attributes.return_puncta_x_position_no_buffer_one_channel(self.tracks, i, channel)
            displacement = np.sqrt((x_pos[1]-x_pos[0])**2 + (y_pos[1]-y_pos[0])**2)
            self.feature_matrix.append(displacement)
